  ...
  return (
    <div className="mb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {pillars.length === 0 ? (
          <div className="col-span-full bg-white/50 backdrop-blur-sm border border-dashed border-gray-300 rounded-2xl p-10 text-center">
            ...
            <button
              onClick={handleAddNew}
              className="bg-white border border-gray-200 text-gray-700 px-6 py-2 rounded-xl font-medium hover:bg-gray-50:bg-gray-600 transition-colors shadow-sm"
            >
              Tambah Sekarang
            </button>
          </div>
          ...
          pillars.map(matkul => (
            <div 
              key={matkul.id} 
              className="group relative bg-white/70 backdrop-blur-md border border-gray-200/50 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all overflow-hidden"
            >
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                <div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-1 line-clamp-1" title={matkul.name}>
                    {matkul.name}
                  </h3>
                  <div className="flex items-center gap-3 text-sm font-medium text-gray-500">
                    <span className="flex items-center gap-1 bg-blue-50 text-blue-600 px-2.5 py-0.5 rounded-full">
                      Semester {matkul.semester || '-'}
                    </span>
                    <span className="flex items-center gap-1 bg-green-50 text-green-600 px-2.5 py-0.5 rounded-full">
                      {matkul.sks || '-'} SKS
                    </span>
                  </div>
                </div>
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="p-1.5 text-gray-400 hover:text-blue-600:text-blue-400 hover:bg-blue-50:bg-blue-900/30 rounded-lg transition-colors"
                  >
                    <Edit3 size={18} />
                  </button>
                  <button 
                    onClick={() => handleDelete(matkul.id, matkul.name)}
                    className="p-1.5 text-gray-400 hover:text-red-600:text-red-400 hover:bg-red-50:bg-red-900/30 rounded-lg transition-colors"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
                ...
                {!matkul.jadwal || (matkul.jadwal as Jadwal[]).length === 0 ? (
                  ...
                  <div className="space-y-2 pl-6">
                    {(matkul.jadwal as Jadwal[]).map((j, i) => (
                      <div key={i} className="flex justify-between items-center text-sm border-l-2 border-gray-200 pl-3">
                        <div>
                          <span className="font-semibold text-gray-700">{j.day}</span>
